    ...
    if args.dir:
        ...
        for dir in directories:
            ...
            for file in files:
                ...
                if perturb:
                    delta = .01*np.random.normal(len(start_params))
                    start_params += delta;
                    os.chdir(GLOBAL_ROOT)
                    if args.backend:
                        circuit = runCircuit(filename, nbits, initial_parameter = start_params,\
                                             loss = args.loss, noisy = args.noisy,\
                                             backend = args.backend)
                        else:
                            circuit = runCircuit(filename, nbits, initial_parameters = start_params, noisy = args.noisy)
